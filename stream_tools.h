...
void StreamingDataWidthConverter_Batch(hls::stream<ap_uint<InWidth> > &in,
    ...
    if (InWidth > OutWidth) {
        ...
        const unsigned int loop_Iters = NumInWords * outPerIn * 1;
        unsigned int o = 0;
...
#pragma HLS PIPELINE II = 1
        ...
        const unsigned int loop_Iters = NumInWords * 1;
...
template <
          unsigned int NumInWords // number of input words to process
          >
void StreamingDataWidthConverter_128to48(hls::stream<ap_uint<128> > &in,
                                         hls::stream<ap_uint<48> > &out,
                                         const unsigned int numReps) {
                                             
    const unsigned int totalIters = NumInWords * 128 / 48 * numReps;
    const unsigned int loop_Iters = NumInWords * 128 / 48 * 1;
    ap_uint<288> ei = 0;
    ap_uint<3> o = 0;
    for (unsigned int t = 0; t < totalIters; t++) {
#pragma HLS LOOP_TRIPCOUNT min=loop_Iters max=loop_Iters
#pragma HLS PIPELINE II = 1
        if (o < 3) {
            ei(127 + 80 * o, 80 * o) = in.read();  // 128-48=80
        }
        // cout<<setbase(16)<<"ei:"<<ei<<endl;
        o++;
        ap_uint<48> eo = ei(47, 0);
        ei = ei >> 48;
        out.write(eo);
        // cout<<setbase(16)<<"eo:"<<eo<<endl;
    }
}
...
template <
          unsigned int NumInWords // number of input words to process
          >
void StreamingDataWidthConverter_128to48_V2(hls::stream<ap_uint<128> > &in,
                                         hls::stream<ap_uint<48> > &out,
                                         const unsigned int numReps) {
...
template <
          unsigned int NumInWords // number of input words to process
          >
void StreamingDataWidthConverter_128to24(hls::stream<ap_uint<128> > &in,
...
#pragma HLS PIPELINE II = 1
        if (o < 3) {
            ei(127 + 104 * o, 104 * o) = in.read(); // 128-24=104
        }
        // cout<<setbase(16)<<"ei:"<<ei<<endl;
        o++;
        ap_uint<24> eo = ei(23, 0);
        ei = ei >> 24;
        out.write(eo);
        // cout<<setbase(16)<<"eo:"<<eo<<endl;
    }
...
template <
          unsigned int NumInWords // number of input words to process
          >
void StreamingDataWidthConverter_64to24(hls::stream<ap_uint<64> > &in,
                                         hls::stream<ap_uint<24> > &out,
                                         const unsigned int numReps) {

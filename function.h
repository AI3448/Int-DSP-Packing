...
template <unsigned IN_BIT, unsigned INC_BIT>
ap_int<IN_BIT + INC_BIT + 1> bn_mul_lut(ap_int<IN_BIT> in, ap_int<INC_BIT> inc) {
...
ap_uint<OUT_BIT> bn_qurelu_fixed(ap_int<IN_BIT> in, ap_int<INC_BIT> inc,
  ...
  ap_int<IN_BIT + INC_BIT + 1> bn_res;
// #pragma HLS RESOURCE variable=bn_res core=Mul_LUT
//   bn_res = in * inc + bias;

  bn_res = bn_mul_lut<IN_BIT, INC_BIT>(in, inc) + bias;
  ap_uint<OUT_BIT> res;

...
void max_pool2x2(stream<ap_uint<PE * IN_BIT * 2>> &vec,
  ...
  const unsigned loop_h = IN_H * 1;
  const unsigned loop_p = IN_CH / PE;
  const unsigned loop_w = IN_W / 2;

  for (unsigned h = 0; h < IN_H * reps; h++)
#pragma HLS LOOP_TRIPCOUNT min=loop_h max=loop_h
    for (unsigned peIdx = 0; peIdx < IN_CH / PE; peIdx++)
#pragma HLS LOOP_TRIPCOUNT min=loop_p max=loop_p
      for (unsigned w = 0; w < IN_W / 2; w++) {
#pragma HLS LOOP_TRIPCOUNT min=loop_w max=loop_w
